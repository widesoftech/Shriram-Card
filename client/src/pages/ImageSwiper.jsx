...
const ImageSwiper = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        const response = await axios.get(
          "https://shriramcard.com/api/v1/template", { withCredentials: true }
        );
        setImages(response.data);
      } catch (error) {
        console.error("Error fetching images:", error);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://shriramcard.com/api/v1/data/getAllEntryes", { withCredentials: true }
        );
        // setData(response.data.data);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://shriramcard.com/api/v1/data/delete/${id}`, { withCredentials: true });
      toast.success("Card Deleted");
      setData((prevData) => prevData.filter((item) => item._id !== id));
    } catch (error) {
      console.error("Error deleting card:", error);
